...
int main(int argc, char **argv)
{
    ...
    */
    ...
    str = (char *) malloc(cmd_args.test_strlen);
    for (ii = 0; ii < cmd_args.test_strlen; ii++) {
        str[ii] = cmd_args.server_name ? 'a' : 'b';
    }
    ...
    if (cmd_args.server_name) {
        ...
        else if (cmd_args.func_put_type == FUNC_PUT_BCOPY) {
            status = do_put_bcopy(ep, &cmd_args, str, *peer_addr, rkey_ob.rkey);
        }
        else if (cmd_args.func_put_type == FUNC_PUT_SHORT) {
            status = do_put_short(ep, &cmd_args, str, *peer_addr, rkey_ob.rkey);
        }
        ...
        if (cmd_args.func_put_type == FUNC_PUT_ZCOPY) {
         ...
         if (cmd_args.func_put_type == FUNC_PUT_BCOPY ||
             cmd_args.func_put_type == FUNC_PUT_SHORT) {
             /* do memory polling */
             while (str[cmd_args.test_strlen - 1] == 'b') {
                 /* do nothing */
             }
        }
    }
