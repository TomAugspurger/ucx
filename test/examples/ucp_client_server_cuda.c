...
static int send_recv_stream(ucp_worker_h ucp_worker, ucp_ep_h ep, int is_server)
{
    ...
    cudaMalloc((void **) &d_recv_message, TEST_STRING_LEN);
    cudaMalloc((void **) &d_test_message, TEST_STRING_LEN);

    cudaMemcpy((void *)d_test_message, (void *)test_message, TEST_STRING_LEN,
               cudaMemcpyHostToDevice);

    if (!is_server) {
        /* Client sends a message to the server using the stream API */
        request = ucp_stream_send_nb(ep, d_test_message, 1,
        ...
        request = ucp_stream_recv_nb(ep, d_recv_message, 1,
                                     ucp_dt_make_contig(TEST_STRING_LEN),
    ...
    cudaMemcpy((void *)recv_message, (void *)d_recv_message, TEST_STRING_LEN,
