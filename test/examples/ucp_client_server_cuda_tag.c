...
static const ucp_tag_t tag  = 0;
static const ucp_tag_t tag_mask = -1;
static int requests_complete = 0;
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;

#define TEST_STRING_LEN sizeof(test_message)
#define MAX_MSG_SIZE (1 << 24)
#define ITERS_MAX (1 << 10)
...
static int send_recv_stream(ucp_worker_h ucp_worker, ucp_ep_h ep, int is_server)
{
    ...
    cudaMalloc((void **) &d_recv_message, MAX_MSG_SIZE);
    cudaMalloc((void **) &d_test_message, MAX_MSG_SIZE);

    for (msg_size = 1; msg_size < MAX_MSG_SIZE; msg_size *= 2) {
        gettimeofday(&t_start, NULL);
        for (iters = 0; iters < ITERS_MAX; iters++) {
            if (!is_server) {
                /* Client sends a message to the server using the stream API */
                request[iters] = ucp_stream_send_nb(ep, test_message, 1,
                                             ucp_dt_make_contig(TEST_STRING_LEN),
                                             stream_send_cb, 0);
                if (UCS_PTR_IS_ERR(request[iters])) {
                    fprintf(stderr, "unable to send UCX message (%s)\n",
                            ucs_status_string(UCS_PTR_STATUS(request[iters])));
                    ret = -1;
                    goto out;
                }
            } else {
                /* Server receives a message from the client using the stream API */
                request[iters] = ucp_stream_recv_nb(ep, &recv_message, 1,
                                             ucp_dt_make_contig(TEST_STRING_LEN),
        ...
        while (requests_complete != (ITERS_MAX - 1)) {
            ucp_worker_progress(ucp_worker);
        }
        gettimeofday(&t_stop, NULL);
        latency = (t_stop.tv_usec - t_start.tv_usec)
            + (1E+6 * (t_stop.tv_sec - t_start.tv_sec));
        printf("%d\t%lf\t%lf\n", msg_size, latency / ITERS_MAX, (msg_size * ITERS_MAX) / latency);
        pthread_mutex_lock(&mutex);
        requests_complete = 0;
        pthread_mutex_unlock(&mutex);
        for (iters = 0; iters < ITERS_MAX; iters++) {
            ucp_request_free(request[iters]);
        }
    }

    cudaFree((void *) d_recv_message);
    cudaFree((void *) d_test_message);
...
static int send_recv_tag(ucp_worker_h ucp_worker, ucp_ep_h ep, int is_server)
{
    ...
    cudaMalloc((void **) &d_test_message, MAX_MSG_SIZE);
#else
    d_test_message = (char *) malloc(MAX_MSG_SIZE);
    d_recv_message = (char *) malloc(MAX_MSG_SIZE);
#endif
    ...
    for (msg_size = 1; msg_size < MAX_MSG_SIZE; msg_size *= 2) {
        ...
        for (iters = 0; iters < ITERS_MAX; iters++) {
            if (!is_server) {
                /* Client sends a message to the server using the stream API */
                request = ucp_tag_send_nb(ep, d_test_message, 1,
                                          ucp_dt_make_contig(msg_size), tag,
                                          send_handle);
                if (UCS_PTR_IS_ERR(request)) {
                    fprintf(stderr, "unable to send UCX address message\n");
                    goto out;
                } else if (UCS_PTR_STATUS(request) != UCS_OK) {
                    wait(ucp_worker, (struct ucx_context *)request);
                    request->completed = 0; /* Reset request state before recycling it */
                    ucp_request_release(request);
                }
            } else {
                request = ucp_tag_recv_nb(ucp_worker, d_recv_message, 1,
                                          ucp_dt_make_contig(msg_size), tag,
...
int main(int argc, char **argv)
{
    ...
    if (server_addr == NULL) {
        ...
        while (1) {
            ...
            send_recv_tag(ucp_worker, context.ep, is_server);
        ...
        ret = send_recv_tag(ucp_worker, ep, is_server);
