                                             ...
                                             uct_completion_t *comp)
{
    ...
    if (ucs_queue_is_empty(&iface->outstanding_d2d_event_q) &&
        ucs_queue_is_empty(&iface->outstanding_h2d_event_q) &&
        ucs_queue_is_empty(&iface->outstanding_d2h_event_q)) {
...
uct_cuda_ipc_progress_event_queue(ucs_queue_head_t *event_queue, unsigned max_events)
{
    ...
    ucs_queue_for_each_safe(cuda_ipc_event, iter, event_queue, queue) {
        cu_ret = cuEventQuery(cuda_ipc_event->event);
...
static unsigned uct_cuda_ipc_iface_progress(uct_iface_h tl_iface)
{
    ...
    count =  uct_cuda_ipc_progress_event_queue(&iface->outstanding_d2d_event_q,
                                              max_events);
    count += uct_cuda_ipc_progress_event_queue(&iface->outstanding_d2h_event_q,
                                              max_events);
    count += uct_cuda_ipc_progress_event_queue(&iface->outstanding_h2d_event_q,
                                              max_events);
    return count;
}
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    for (i = 0; i < dev_count; i++) {
        for (j = 0; j < dev_count; j++) {
            cu_ret = cuDeviceCanAccessPeer(&(self->p2p_map[i][j]),
    ...
    ucs_queue_head_init(&self->outstanding_h2d_event_q);
    ucs_queue_head_init(&self->outstanding_d2h_event_q);
